...
class EmployeeDashboard(models.AbstractModel):
    ...
    def _get_employee_hierarchy(self, employee):
        ...
        query = """WITH RECURSIVE emp_tree AS (
                    SELECT e.id, e.parent_id, e.name, e.job_id FROM hr_employee e WHERE e.id = %s
                    UNION ALL
                    SELECT e.id, e.parent_id, e.name, e.job_id FROM hr_employee e
                    INNER JOIN emp_tree et ON e.parent_id = et.id)
                SELECT et.id, et.parent_id, et.name, j.name AS job_name FROM emp_tree et
                LEFT JOIN hr_job j ON et.job_id = j.id
                UNION
                SELECT m.id, m.parent_id, m.name, j.name AS job_name FROM hr_employee m
                LEFT JOIN hr_job j ON m.job_id = j.id WHERE m.id = (SELECT parent_id FROM hr_employee WHERE id = %s)
                """
    ...
    def _get_manager_data(self, filter_date):
        ...
        # Attendance Counts
        cr.execute("""
            SELECT DISTINCT a.employee_id FROM hr_attendance a
        ...
        cr.execute("""
            SELECT e.gender, COUNT(DISTINCT a.employee_id) FROM hr_attendance a
            JOIN hr_employee e ON a.employee_id = e.id
            WHERE a.check_in >= %s AND a.check_in <= %s
            GROUP BY e.gender
        """, (start_dt, end_dt))
        ...
        # Leave Counts
        cr.execute("""
            SELECT SUM(l.number_of_days), e.gender FROM hr_leave l
            JOIN hr_employee e ON l.employee_id = e.id
            WHERE l.request_date_from <= %s AND l.request_date_to >= %s AND l.state = %s
            GROUP BY e.gender
        ...
        cr.execute("""
            ...
            WHERE active = %s AND last_update_status != %s
        """, (True, 'done'))
        remaining_projects = cr.fetchone()[0]

        cr.execute("""
            SELECT COUNT(*) FROM project_task_user_rel rel
            JOIN project_task t ON rel.task_id = t.id
            JOIN project_task_type stage ON t.stage_id = stage.id
            WHERE t.active = TRUE AND (stage.fold = FALSE OR stage.fold IS NULL)
        """)
    ...
    def _get_attendance_data(self, employee, start, end):
        ...
        cr.execute("""
            ...
            WHERE employee_id = %s AND check_in IS NOT NULL AND check_out IS NULL
    ...
    def _get_leave_data(self, employee, start, end):
        ...
        cr.execute("""
            ...
            WHERE employee_id = %s AND state = 'validate' AND request_date_to >= %s AND request_date_from <= %s
        ...
        cr.execute("""
            ...
            WHERE employee_id = %s AND state IN ('confirm', 'validate1')
        """, (employee.id,))
    ...
    def _get_project_tasks(self, employee):
        ...
        cr.execute("""
            ...
            LEFT JOIN project_project p ON t.project_id = p.id
            LEFT JOIN project_task_type stage ON t.stage_id = stage.id
            WHERE rel.user_id = %s AND t.active = TRUE
            ORDER BY t.date_deadline ASC NULLS LAST, t.priority DESC NULLS LAST
        """, (user_id,))
    ...
    def _get_project_tasks_count(self, employee):
        ...
        cr.execute("""
            ...
            JOIN project_task t ON rel.task_id = t.id
            JOIN project_task_type stage ON t.stage_id = stage.id
            WHERE rel.user_id = %s AND t.active = TRUE AND (stage.fold = FALSE OR stage.fold IS NULL)
        """, (user_id,))
        remaining_task_count = cr.fetchone()[0]

        cr.execute("SELECT COUNT(*) FROM project_project WHERE user_id = %s AND active = TRUE", (user_id,))
        project_count = cr.fetchone()[0]

        cr.execute("""
            SELECT COUNT(*) FROM project_project
            WHERE user_id = %s AND active = TRUE AND (last_update_status != 'done' OR last_update_status IS NULL)
        """, (user_id,))
